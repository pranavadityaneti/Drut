...
-- the free-tier 15/day quota. Pro users bypass this in app code.
--
-- Increment: usePaywall() hook on web + mobile calls increment_daily_question_usage()
--            after the user answers a question.
-- Read: paywall modal checks count vs limit before letting the next question render.
--
-- IMPORTANT: the limit (15) is enforced in app code, NOT in the DB, so we can
-- A/B test or relax it without a migration. The DB only tracks the count.
...
-- Users can read/write only their own usage rows. Write path is the RPC below.
create policy "daily_question_usage_select_own"
  on public.daily_question_usage for select
  using (auth.uid() = user_id);

create policy "daily_question_usage_insert_own"
  on public.daily_question_usage for insert
  with check (auth.uid() = user_id);

create policy "daily_question_usage_update_own"
  on public.daily_question_usage for update
  using (auth.uid() = user_id);
...
comment on table public.daily_question_usage is 'Per-user per-day question counts for free-tier 15/day quota enforcement. Limit lives in app code, not here.';
