...
export const BulkIngest: React.FC = () => {
  ...
  <FileSpreadsheet className="h-5 w-5 text-[#3d7a0f]" />
  CSV Upload
  </CardTitle>
  <CardDescription>
  ...
  ${isDragActive
  ? 'ring-2 ring-[var(--color-primary)] bg-[var(--color-accent)]'
  : 'border-[var(--color-ink-5)] hover:border-emerald-300 hover:bg-[var(--color-accent)]/50'}
  `}
  >
  <input {...getInputProps()} />
  <Upload className="h-10 w-10 mx-auto text-[var(--color-ink-3)] mb-4" />
  <p className="text-[var(--color-ink-2)] font-medium">
  ...
  {/* Parse Error */}
  {parseError && (
  <div className="flex items-start gap-3 p-4 bg-[#fde7e5] border border-[#fde7e5] rounded-xl">
  <AlertCircle className="h-5 w-5 text-[var(--color-destructive)] flex-shrink-0 mt-0.5" />
  <div>
  <p className="text-[var(--color-destructive)] font-medium">Parse Error</p>
  <pre className="text-sm text-[var(--color-destructive)] mt-1 whitespace-pre-wrap">{parseError}</pre>
  </div>
  </div>
  )}
  ...
  <tr>
  <th className="px-4 py-3 text-left text-xs font-medium text-[var(--color-ink-3)] uppercase tracking-wider">#</th>
  <th className="px-4 py-3 text-left text-xs font-medium text-[var(--color-ink-3)] uppercase tracking-wider">Topic</th>
  <th className="px-4 py-3 text-left text-xs font-medium text-[var(--color-ink-3)] uppercase tracking-wider">Question</th>
  <th className="px-4 py-3 text-left text-xs font-medium text-[var(--color-ink-3)] uppercase tracking-wider">FSM Tag</th>
  </tr>
  </thead>
  <tbody className="bg-white divide-y divide-gray-200">
  {parsedQuestions.slice(0, 5).map((q, i) => (
  <tr key={i}>
  <td className="px-4 py-3 text-sm text-[var(--color-ink-3)]">{i + 1}</td>
  <td className="px-4 py-3 text-sm text-[var(--color-ink-1)]">{q.subtopic}</td>
  <td className="px-4 py-3 text-sm text-[var(--color-ink-1)] max-w-md truncate">{q.question_text}</td>
  <td className="px-4 py-3">
  <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-[var(--color-accent)] text-[#3d7a0f]">
  {q.fsm_tag}
  </span>
  </td>
  </tr>
  ...
  <div className="px-4 py-2 bg-[var(--color-muted)] text-sm text-[var(--color-ink-3)]">
  ...
  disabled={isUploading}
  className="w-full bg-[#3d7a0f] hover:bg-[#3d7a0f] text-white"
  >
  {isUploading ? (
  <>
  ...
  <span className="text-[var(--color-ink-2)]">{progress.current}</span>
  <span className="font-medium">{progress.processed}/{progress.total}</span>
  </div>
  <div className="h-2 bg-[var(--color-ink-5)] rounded-full overflow-hidden">
  <div
  className="h-full bg-[#3d7a0f] transition-all duration-300"
  ...
  <CheckCircle2 className="h-16 w-16 mx-auto text-[var(--color-primary)] mb-4" />
  <h3 className="text-xl font-bold text-[var(--color-ink-1)] mb-2">Upload Complete!</h3>
  <p className="text-[var(--color-ink-2)] mb-4">
  Successfully processed <span className="font-medium text-[#3d7a0f]">{progress.succeeded}</span> questions
  ...
  <CardContent>
  <pre className="bg-[var(--color-muted)] p-4 rounded-lg overflow-x-auto text-sm">
  {SAMPLE_CSV}
  </pre>
  <Button
  variant="ghost"
