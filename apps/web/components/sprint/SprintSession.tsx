...
export const SprintSession: React.FC<SprintSessionProps> = ({ config, onExit }) => {
 ...
 <div>
 <p className="text-sm font-bold text-[var(--color-ink-3)] uppercase tracking-widest">Question {currentIndex + 1} / {questions.length}</p>
 <div className="h-1.5 w-64 bg-[var(--color-muted)] rounded-full mt-2 overflow-hidden">
 <div className="h-full bg-[var(--color-ink-1)] transition-all duration-300" style={{ width: `${progress}%` }} />
 </div>
 </div>
 <div className="text-right">
 <p className="text-sm font-bold text-[var(--color-ink-3)] uppercase tracking-widest">Score</p>
 <p className="text-3xl font-black text-[#3d7a0f]">{stateRef.current.score}</p>
 </div>
 </div>

 {/* Timer Bar */}
 <Card className={`border-2 transition-colors duration-300 ${isUrgent ? 'border-red-500 shadow-red-100' : 'border-[var(--color-primary)] shadow-emerald-50'} shadow-lg`}>
 <CardContent className="p-0">
 <div
 className={`h-2 transition-all duration-75 ease-linear ${isUrgent ? 'bg-[var(--color-destructive)]' : 'bg-[#3d7a0f]'}`}
 style={{ width: `${timeProgress}%` }}
 />
 <div className="p-4 flex justify-between items-center">
 <span className={`text-2xl font-mono font-bold ${isUrgent ? 'text-[var(--color-destructive)] animate-pulse' : 'text-[#3d7a0f]'}`}>
 {timeLeft.toFixed(1)}s
 </span>
 <span className="text-xs font-bold text-[var(--color-ink-3)] uppercase tracking-wider">Time Remaining</span>
 </div>
 </CardContent>
 </Card>
 ...
 <span className="flex-shrink-0 w-8 h-8 flex items-center justify-center rounded-lg bg-[var(--color-muted)] text-[var(--color-ink-3)] font-bold group-hover:bg-[var(--color-muted)] group-hover:text-[var(--color-ink-2)] transition-colors">
