 ...
 *
 * MOBILE: do NOT use this hook on React Native — there's no DOM. Mobile
 * uses react-native-razorpay which provides RazorpayCheckout.open() directly.
 * That mobile entrypoint will live alongside this in a separate file.
 */
...
export interface UseRazorpayCheckoutOptions {
    ...
    image?: string;
    /** Theme color (CTA + accents). Defaults to Drut lime. */
    themeColor?: string;
...
export function useRazorpayCheckout(opts: UseRazorpayCheckoutOptions = {}): UseRazorpayCheckoutReturn {
    ...
    const pay = useCallback(async (plan: PlanId): Promise<VerifyPaymentResponse> => {
        ...
        try {
            ...
            // 2. Create the order via our edge fn.
